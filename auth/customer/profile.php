...
<?php
    $user_q = $db->query("SELECT * FROM customers WHERE id=$user_id");
    $user = $user_q->fetch_assoc();

    if(isset($_POST['email'])){

        if (!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
            echo "<script>alert('Ops! invalid email!');window.location='profile.php '</script>";
            exit();
        }

        $customer_q = $db->query("SELECT * FROM customers WHERE email='$_POST[email]' AND id !=$user_id");
        ...
        if($customer || $agent || $admin){
            echo "<script>alert('Email already exist!');window.location='profile.php'</script>";
            exit();
        }

        $name = strtoupper($_POST['name']);
        $update = $db->query("UPDATE customers SET email='$_POST[email]', phone_number='$_POST[phone_number]',name='$name' WHERE id=$user_id");
        if(!$update){
            dd($db->error);
        }
    ...
    if(isset($_POST['password'])){
        ...
        if($new_password != $confirm_password){
            echo "<script>alert('Confirm password not match!');window.location='profile.php'</script>";
            exit();
        }

        if($x == $new_password){
            echo "<script>alert('Do not use old password!');window.location='profile.php'</script>";
            exit();
        }

        $hash_pass = password_hash($new_password, PASSWORD_BCRYPT);
        $update = $db->query("UPDATE customers SET password='$hash_pass' WHERE id=$user_id");
        if(!$update){
            dd($db->error);
        }
